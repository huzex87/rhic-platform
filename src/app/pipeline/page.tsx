        ...
        <div className="max-w-7xl mx-auto px-4 space-y-12">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                ...
                <div className="flex gap-4">
                    <button className="leaf-gradient text-ivory px-8 py-4 rounded-xl font-black shadow-xl hover:scale-105 transition-all flex items-center gap-2">
                ...
                <div className="lg:col-span-2 space-y-8">
                    <div className="flex gap-8 border-b border-forest/5 pb-4">
                        {['Trending', 'Most Voted', 'Recent', 'In Review'].map((tab, i) => (
                            <button key={tab} className={`text-sm font-bold tracking-tight transition-all ${i === 0 ? 'text-forest border-b-2 border-leaf pb-4 -mb-[18px]' : 'text-forest/40 hover:text-forest'}`}>
                                {tab}
                            </button>
                        ))}
                    </div>
                            ...
                            >
                                <div className="flex gap-6">
                                    <div className="flex flex-col items-center gap-2">
                                        <button className="p-3 rounded-xl bg-forest/5 hover:bg-leaf hover:text-ivory transition-all text-forest">
                                            <ArrowUp className="w-5 h-5" />
                                        </button>
                                        <span className="text-sm font-black text-forest">{idea.votes}</span>
                                    </div>
                                    ...
                                    <div className="flex-1 space-y-3">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <span className="text-[10px] font-bold text-forest/30 uppercase tracking-widest">By {idea.author}</span>
                                        </div>
                                        ...
                                        <div className="flex items-center justify-between pt-4">
                                            ...
                                            <div className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${idea.status === 'Reviewing' ? 'bg-orange-50 text-orange-600' :
                                                idea.status === 'Shortlisted' ? 'bg-leaf/10 text-leaf' : 'bg-forest/5 text-forest/40'
                                                }`}>
                                                {idea.status}
                                            </div>
                                        </div>
