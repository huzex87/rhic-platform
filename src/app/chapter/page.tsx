                ...
                <div className="lg:col-span-2 space-y-6">
                    <div className="premium-card">
                        <div className="flex items-center justify-between mb-8">
                            <h3 className="text-xl font-display font-bold text-forest">LGA Coordinators</h3>
                            <div className="text-xs font-bold text-forest/20 uppercase">Showing 20 representatives</div>
                        </div>
                        <div className="overflow-x-auto">
                            <table className="w-full text-left">
                                <thead>
                                    <tr className="border-b border-forest/5">
                                        <th className="pb-4 text-[10px] font-black text-forest/30 uppercase tracking-[0.2em]">LGA Name</th>
                                        <th className="pb-4 text-[10px] font-black text-forest/30 uppercase tracking-[0.2em]">Coordinator</th>
                                        <th className="pb-4 text-[10px] font-black text-forest/30 uppercase tracking-[0.2em]">Recruitment</th>
                                ...
                                <tbody className="divide-y divide-forest/5">
                                    {[
                                        ...
                                        <tr key={i} className="hover:bg-forest/5 transition-colors group">
                                            <td className="py-5 font-bold text-forest">{item.lga}</td>
                                            <td className="py-5">
                                                <div className="flex items-center gap-3">
                                                    <div className="w-8 h-8 rounded-full bg-forest/10 flex items-center justify-center text-[10px] font-black">
                                                        {item.rep.split(' ').map(n => n[0]).join('')}
                                                    </div>
                                                    <span className="text-sm font-medium text-forest/80">{item.rep}</span>
                                                </div>
                                            </td>
                                            <td className="py-5 font-black text-forest">{item.count.toLocaleString()}</td>
                                            <td className="py-5">
                                                <span className={`px-2 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest ${item.status === 'Active' ? 'bg-leaf/10 text-leaf' : 'bg-accent-red/10 text-accent-red'
                                                    }`}>
                                                    {item.status}
                                                </span>
                                            </td>
