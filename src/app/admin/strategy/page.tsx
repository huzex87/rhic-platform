        ...
        <div className="max-w-7xl mx-auto px-4 space-y-8 pb-12">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div>
                    <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-[10px] font-black text-accent-red uppercase tracking-[0.2em]">National Operational Intelligence</span>
                    </div>
                    <h1 className="text-4xl md:text-5xl font-black text-forest tracking-tighter">
                        Strategy <span className="text-leaf">Room</span>
                    </h1>
                    <p className="text-forest/60 font-medium mt-2 max-w-xl">
                        Real-time aggregate visualization of national mobilization momentum, sentiment analysis, and regional readiness.
                    </p>
                </div>

                <div className="flex bg-forest/5 p-1 rounded-2xl border border-forest/10 p-1.5 backdrop-blur-sm">
                    {[
                        { id: 'overview', icon: BarChart3, label: 'Overview' },
                        { id: 'geospatial', icon: MapIcon, label: 'Geo-Strategy' },
                        { id: 'analytics', icon: TrendingUp, label: 'Forecast' }
                    ].map((tab) => (
                        <button
                            key={tab.id}
                            ...
                            className={`flex items-center gap-2 px-6 py-3 rounded-xl font-bold transition-all ${activeTab === tab.id
                                ? 'bg-ivory text-forest shadow-lg scale-[1.02]'
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 premium-card overflow-hidden">
                    ...
                    <div className="h-[500px] w-full bg-forest/5 rounded-3xl flex items-center justify-center border border-forest/10 relative overflow-hidden group">
                        ...
                        <div className="relative z-10 text-center space-y-4">
                            <div className="w-16 h-16 bg-ivory rounded-2xl shadow-xl flex items-center justify-center mx-auto border border-forest/5">
                                <MapIcon className="w-8 h-8 text-leaf" />
                            </div>
                            <div>
                                <h4 className="text-lg font-black text-forest">Interactive Strategy Map</h4>
                                <p className="text-sm text-forest/60 font-medium max-w-xs px-6">Select a region to deep-dive into LGA readiness and volunteer saturation.</p>
                            </div>
                            <button className="px-8 py-3 bg-forest text-ivory rounded-xl font-bold text-sm hover:bg-leaf transition-colors shadow-lg">
                                Activate Live Map
                            </button>
                        </div>
                    </div>
                ...
                <div className="space-y-8">
                    ...
                    <div className="premium-card bg-forest text-ivory border-none">
                        <div className="w-12 h-12 bg-ivory/10 rounded-xl flex items-center justify-center mb-6">
                            <ShieldAlert className="w-6 h-6 text-leaf" />
                        </div>
                        <h3 className="text-xl font-black mb-2">Security Audit</h3>
                        <p className="text-ivory/60 text-sm font-medium mb-6 leading-relaxed">
                            All data streamed into the Strategy Room is encrypted and subject to Command Level 5 RLS policies.
                        </p>
                        <div className="flex items-center gap-2 group cursor-pointer">
                            <span className="text-xs font-bold text-leaf tracking-widest uppercase">Full Compliance Logs</span>
                            <TrendingUp className="w-4 h-4 text-leaf group-hover:translate-x-1 transition-transform" />
                        </div>
                    </div>
                </div>
            </div>
