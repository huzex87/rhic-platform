            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div>
                    <h1 className="text-4xl font-display font-black text-navy leading-tight">
                        Mobilization <span className="text-gold italic">Situation Room</span>
                    </h1>
                    <p className="text-navy/60 font-medium">Real-time national momentum and chapter performance metrics.</p>
                </div>
                <div className="flex gap-4">
                    <button className="glass px-6 py-3 rounded-xl font-bold text-navy hover:bg-white transition-all flex items-center gap-2">
                        Export Report
                    </button>
                    <button className="navy-gradient text-ivory px-6 py-3 rounded-xl font-bold shadow-lg hover:shadow-xl transition-all flex items-center gap-2">
                ...
                <div className="lg:col-span-2 space-y-8">
                    <div className="premium-card min-h-[500px] flex flex-col justify-between overflow-hidden relative">
                        <div className="flex justify-between items-start z-10">
                            <div>
                                <h2 className="text-xl font-display font-bold text-navy">National Reach</h2>
                                <div className="flex items-center gap-2 text-xs font-bold text-navy/40 uppercase tracking-widest mt-1">
                                    <Activity className="w-3 h-3 text-gold" />
                                    Live Activity Mapping
                                </div>
                            </div>
                            <div className="glass px-3 py-1 rounded-lg text-xs font-black text-navy">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div className="h-40 flex items-end gap-2 px-2">
                                {[40, 70, 45, 90, 65, 80, 55].map((h, i) => (
                                    <motion.div
                                        ...
                                        initial={{ height: 0 }}
                                        animate={{ height: `${h}%` }}
                                        className="flex-1 bg-navy/10 rounded-t-md hover:bg-gold/40 transition-colors relative group"
                                    >
                                        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 bg-navy text-ivory text-[10px] px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
                                            {h}k new members
                                        </div>
                                    </motion.div>
                                ))}
                            </div>
                            <div className="flex justify-between mt-4 text-[10px] font-bold text-navy/20 uppercase">
                                <span>Mon</span>
                                <span>Sun</span>
                            </div>
                        </div>
                        ...
                        <div className="premium-card">
                            <h3 className="text-sm font-bold text-navy/40 uppercase tracking-widest mb-6">Recent Deployments</h3>
                                    ...
                                    <div key={i} className="flex items-center justify-between p-3 rounded-xl bg-navy/5 hover:bg-navy/10 transition-colors group cursor-pointer">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <div className="text-[10px] text-navy/40 font-bold uppercase">{item.state}</div>
                                            </div>
                                        </div>
                                        <div className="text-[10px] font-bold text-navy/30 group-hover:text-navy transition-colors">{item.date}</div>
                                    </div>
                                ))}
                            </div>
                        </div>
                ...
                <div className="space-y-8">
                    <div className="premium-card navy-gradient text-ivory relative overflow-hidden">
                        <div className="relative z-10">
                            <h3 className="text-xs font-bold text-gold uppercase tracking-[0.2em] mb-4">Active Campaign</h3>
                            ...
                            <button className="bg-gold text-navy w-full py-4 rounded-xl font-black shadow-xl hover:scale-[1.02] transition-all">
                                JOIN CHALLENGE
                            </button>
                        </div>
                    ...
                    <div className="premium-card">
                        <div className="flex justify-between items-center mb-6">
                            <h3 className="text-sm font-bold text-navy/40 uppercase tracking-widest">Top Performers</h3>
                            ...
                            {[
                                ...
                                <div key={item.name} className="flex items-center justify-between">
                                    <div className="flex items-center gap-4">
                                        <div className="w-8 h-8 rounded-lg bg-navy/5 flex items-center justify-center font-black text-navy text-sm">
                                            {item.rank}
                                        </div>
                                        <div>
                                            <div className="text-sm font-black text-navy">{item.name}</div>
                                            <div className="text-[10px] font-bold text-navy/30 uppercase tracking-widest">{item.points} Points</div>
                                        </div>
                                    </div>
                                    <div className={`w-2 h-2 rounded-full ${item.trend === 'up' ? 'bg-green-500' : 'bg-red-500'} shadow-sm`} />
                                </div>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="premium-card bg-ivory/50">
                        <h3 className="text-sm font-bold text-navy/40 uppercase tracking-widest mb-6">Quick Actions</h3>
                        <div className="grid grid-cols-2 gap-4">
                            {[
                                ...
                                <button key={action.label} className="flex flex-col items-center gap-2 p-4 rounded-2xl bg-white border border-navy/5 hover:border-gold/50 hover:shadow-lg transition-all text-navy group">
                                    <action.icon className="w-5 h-5 group-hover:text-gold transition-colors" />
